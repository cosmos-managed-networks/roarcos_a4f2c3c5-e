...
# then log output of file /code/bin/roarcos
# then ensure static linking
RUN LEDGER_ENABLED=false BUILD_TAGS=muslc LINK_STATICALLY=true make build \
  && file /code/build/roarcos \
  && echo "Ensuring binary is statically linked ..." \
  && (file /code/build/roarcos | grep "statically linked")

# --------------------------------------------------------
FROM alpine:3.16

COPY --from=go-builder /code/build/roarcos /usr/bin/roarcos
