...
HTTPS_GIT := github.com/roarcos/roarcos.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=roarcos \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=roarcos \
...
ifeq ($(OS),Windows_NT)
	...
	go build -mod=readonly $(BUILD_FLAGS) -o build/roarcos ./cmd/roarcos
endif

build-windows-client: go.sum
	GOOS=windows GOARCH=amd64 go build -mod=readonly $(BUILD_FLAGS) -o build/roarcos.exe ./cmd/roarcos
...
install: go.sum
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/roarcos
...
draw-deps:
	...
	@goviz -i ./cmd/roarcos -d 2 | dot -Tpng -o dependency-graph.png
...
ifeq (,$(shell which heighliner))
	...
	heighliner build -c roarcos --local -f chains.yaml
endif
...
set-testnet-configs:
	roarcos config set client chain-id chainid-1
	roarcos config set client keyring-backend test
	roarcos config set client output text

# import keys from testnet.json into test keyring
setup-testnet-keys:
	-`echo "decorate bright ozone fork gallery riot bus exhaust worth way bone indoor calm squirrel merry zero scheme cotton until shop any excess stage laundry" | roarcos keys add acc0 --recover`
	-`echo "wealth flavor believe regret funny network recall kiss grape useless pepper cram hint member few certain unveil rather brick bargain curious require crowd raise" | roarcos keys add acc1 --recover`
